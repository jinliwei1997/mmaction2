...
class VideoSubtitleAudioTextMatcherE2E(nn.Module):
    ...
    def __init__(self,
        ...
        self.img_subtitle_audio_mlp = nn.Sequential(nn.Linear(img_feat_dim + text_feat_dim + audio_feat_dim, self.feature_dim * 2), nn.BatchNorm1d(self.feature_dim * 2), nn.ReLU(), nn.Linear(self.feature_dim * 2, self.feature_dim))
    ...
    def forward(self, imgs, subtitle_texts_item, audios, texts_item, return_loss=True):
        """Define the computation performed at every call."""
        if return_loss:
            return self.forward_train(imgs, subtitle_texts_item, audios, texts_item)

        return self.forward_test(imgs, subtitle_texts_item, audios, texts_item)

    def forward_train(self, imgs, subtitle_texts_item, audios, texts_item):
        ...
        return self.head(v_s_a_feat, t_feat)

    def forward_test(self, imgs, subtitle_texts_item, audios, texts_item):
        N = imgs.shape[0]
        imgs = imgs.reshape((-1,) + imgs.shape[2:])
        v_feat = self.encoder_v(imgs, N)
        ...
        for key in texts_item:
            texts_item[key] = texts_item[key].reshape((-1,) + texts_item[key].shape[2:])
        t_feat = nn.functional.normalize(self.text_mlp(self.encoder_t(texts_item)),
                                         dim=1)  # [N * text_num_per_video (T), C]

        return zip(v_s_a_feat.cpu().numpy(),t_feat.view(N, -1, t_feat.shape[1]).cpu().numpy())
